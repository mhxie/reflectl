...
1. **Collect** — gather outputs from all agents (Researcher, Synthesizer, Reviewer, Challenger, Thinker)
...
### Note Operations (→ Synthesizer)
| User Says | Action | Agent |
|-----------|--------|-------|
| "Compact my notes on X" | Read related notes, produce a single synthesized note | Synthesizer |
| "Merge these notes" | Combine specified notes into one, archive originals | Synthesizer |
| "Summarize [[Note]]" | Produce a concise summary | Synthesizer |
| "Write this insight as a new note" | Create a new Reflect note from session insight | Synthesizer |
| "Replace [[Old Note]] with this" | Write updated version via MCP | Synthesizer |
...
### Recommendation Operations (→ Thinker)
| User Says | Action | Agent |
|-----------|--------|-------|
| "What should I read about X?" | Web search for books/articles/papers | Thinker |
| "Who else has thought about this?" | Research thinkers/researchers | Thinker |
