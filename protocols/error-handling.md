...
- **Reflect MCP unreachable**: No effect on the read path — `zk/` is primary. Degrades only (a) today's fresh daily note before the sync catches up (fall back to yesterday's local file and warn), (b) `/weekly`'s per-day fallback for older missing/empty/truncated daily notes (skip the MCP fetch, warn the user about the gap, and proceed with the local notes that are present rather than blocking the weekly review), (c) write-back via `append_to_daily_note` (save the write-back to `zk/reflections/` and inform the user), and (d) `create_note` dispatches (Curator queues the proposed note as a local draft under `zk/drafts/` for later retry).
...
- **Local mirror stale across past 7 days** (`/weekly` invoked from `/reflect`'s hard-floor cue while Background Sync is mid-merge, or sync was incomplete): the orchestrator may fall through to `get_daily_note(date: "<that-date>")` for any of the past 7 days that is missing/empty/truncated, then merge into the local file via `scripts/merge_daily.py` (never overwrite — daily notes can hold local-only edits). This per-day fallback is `/weekly`-specific and is documented in `.claude/commands/weekly.md` Context Loading.
- **Target note genuinely missing from local mirror**: report the gap honestly. There is no generic `get_note` / `search_notes` fallback — subagents do not have these tools. The orchestrator-side `get_note` calls that remain are narrow: (a) the curator snapshot flow in `protocols/agent-handoff.md`, (b) verification of a manual wiki-entry share after `create_note` (empty-body guard), and (c) `/restore` emergency recovery when the user supplies a Reflect note ID. Nothing else.
