...
- **`$ZK/` (the vault, typically mounted under cloud storage)** is the **structured repository** — single source of truth for organized data.
- After ingestion, originals live only in `$ZK/<domain>/raw/` (or in the vault's gitignored cache for entities not yet promoted). Drive top-level domain folders and Downloads are transient.
...
7. **rmdir empty source folders**, including the Drive top-level domain folder if fully drained. When `$ZK` lives in cloud storage, files in `$ZK` remain in the cloud; no data leaves it.
...
- Non-self entities (companion's medical, foster pets, etc.) get their own subdirectory under the domain (e.g., `$ZK/health/pet/`). Same protocol applies internally; cross-links from main self-line note "see also" but don't merge data.
