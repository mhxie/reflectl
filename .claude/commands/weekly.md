...
3. **Read the past 7 daily notes from the local mirror:**
   - `Read zk/daily-notes/<today>.md` through `Read zk/daily-notes/<7-days-ago>.md` (7 local reads). **For any file that is missing, empty, or visibly truncated, fall through via the same protocol as the Background Daily-Notes Sync: dispatch `get_daily_note(date: "<that-date>")` via MCP, write the response body to `zk/cache/reflect-daily-<that-date>.md`, then run `Bash: uv run scripts/merge_daily.py zk/daily-notes/<that-date>.md zk/cache/reflect-daily-<that-date>.md`** to merge with any local edits. Do not overwrite the local file directly — `merge_daily.py` is the only safe write path because daily notes can hold local-only edits the user has not yet pushed to Reflect (Orchestrator only). This per-day fallback is broader than the daily-reflection fallback (which covers only today) because `/weekly` may be invoked from `/reflect`'s hard-floor cue route while the Background Daily-Notes Sync is still running, and any of the past 7 days may be incomplete.
